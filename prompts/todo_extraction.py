...
def get_todo_extraction_prompt(messages: List[Dict[str, Any]]) -> str:
    """
    ...
    prompt = f"""Analyze the following Slack messages and extract all todos, tasks, action items, or things that need to be done.

Messages:
{messages_text}

Extract all actionable items from these messages. Be thorough and extract all todos, even if they're mentioned casually. For each todo, identify:
